...
export class StockMovementDialogComponent implements OnInit {
  ...
  private readonly dialogRef = inject(MatDialogRef<StockMovementDialogComponent>);
  ...
  getError(field: string): string {
    ...
    if (ctrl.hasError('min')) return `La cantidad mínima es ${ctrl.errors?.['min'].min}`;
  ...
  onSubmit(): void {
    ...
    this.movementService.register(request).subscribe({
      ...
      error: (err) => console.error('Error registrando movimiento', err)
    });
  }
